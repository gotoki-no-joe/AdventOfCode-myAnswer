...
near8 (x,y) (hx,hy) = abs (x - hx) <= 1 && abs (y - hy) <= 1

follow t@(tx,ty) h@(hx,hy)
  | near8 t h = t
  | not (null cands1) = head cands1
  | otherwise = snd $ minimum [(abs (tx-cx) + abs (ty-cy), (cx,cy)) | cx <- [pred hx..succ hx], cy <- [pred hy..succ hy]]
  where
    cands1 = [c | c <- map (add h) dir4, near8 t c]
...
あかん。
斜めのときに同じ斜めに移動されると追いつかない。
そういうときは、目標の8近傍で最も近い位置、とかかしら。
最初からそれではないのがなぁ。
