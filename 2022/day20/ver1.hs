...
-- 前
backward :: Int -> DLL -> IO Int
backward x (_,vB) = MUV.read vB x
...
-- i個めの要素を d だけ移動させる、その移動先を、リンクを辿って特定する
    case compare d 0 of
      EQ -> return ()
      LT -> do
        j <- foldM (\j _ -> backward j dll) i [d .. 0]
        deleteDLL i dll
        insertDLL j i dll
      GT -> do
        j <- foldM (\j _ -> forward j dll) i [1 .. d]
        deleteDLL i dll
        insertDLL j i dll
    )
