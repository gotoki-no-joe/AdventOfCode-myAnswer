...
interpret pc mem js = case opcode of
    1 -> arith (+)
    2 -> arith (*)
    3 -> interpret (pc + 2) (mem // [(fromIntegral $ ma (pc + 1), head js)]) (tail js)
...
-- </new>
  ...
  where
    ...
    arith op = interpret (pc + 4) (mem // [(fromIntegral $ ma (pc + 3), op av1 av2)]) js
    jump True  = interpret (fromIntegral av2) mem js
    jump False = interpret (pc + 3) mem js
    set1 b = interpret (pc + 4) (mem // [(fromIntegral $ ma (pc +3),if b then 1 else 0)]) js
{-
*Main> main
0
0
