...
後半は、長さ最長のものだけ考えろということで、長さも追加する。
-}
...
part1 ps = maximum $ dfs step (0,avail0,0,0)
  where
    n = length ps
    iedges = zip [1..] ps
    ...
    step (node, avails, len, score) = ([(len,score)],[(a,IS.delete e avails,succ len,score1+a) |(e,a) <- dests])
      where
        score1 = score + node
