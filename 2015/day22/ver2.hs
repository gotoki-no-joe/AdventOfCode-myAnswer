...
compute1 :: Int -- 最小値
         -> [Status] -- 調べる状態リスト
         -> Int -- 結果
compute1 curTotal [] = curTotal
--compute1 curTotal stats
--  | trace (show curTotal ++ " " ++ show (length stats)) $ False = undefined
compute1 curTotal (stat:stats)
  ...
  | True = (compute1 $! ct2) $! conts2 where
    ...
    conts2 = foldr insert stats conts

ans1 = compute1 maxBound [initial]

test x = compute1 maxBound [initial { bhp = x } ]

{-
test 10, 20, 30 は一瞬、40は少し待たされて、ans1は完全に止まる。
その辺で状態爆発するということだよね。
放置したら返ってきた。
...
compute2 :: Int -- 最小値
         -> [Status] -- 調べる状態リスト
         -> Int -- 結果
compute2 curTotal [] = curTotal
--compute2 curTotal stats
--  | trace (show curTotal ++ " " ++ show (length stats)) $ False = undefined
compute2 curTotal (stat:stats)
  | curTotal < total stat = curTotal
...
ans2 = compute2 maxBound [hardinitial]

test2 x = compute2 maxBound [hardinitial { bhp = x } ]

{-
速度感は変わらないから、待てばこっちも答え出るはずだよね。
ヒープ使って効率化したら少しは待ちが短くなるはずだけども。
